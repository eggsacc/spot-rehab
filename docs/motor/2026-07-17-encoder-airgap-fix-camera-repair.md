# Left hip-X healthy: chip #2 + air-gap fix; rear camera firmware transplant
...
**Objective:**
>Rebuild the left hind hip-X socket (replace the chip on the eR PCB and correct the air gap) to test the disc/air-gap hypothesis from the [15-07 board swap](2026-07-15-encoder-board-swap-spotcheck3.md). simultaneously repair the rear depth camera (StereoProto fault since 18-06) by firmware transplant and verify both with SpotCheck.
...
`NEW1` = original eL PCB + third-party iC-MU chip ([14-07 rework](2026-07-14-encoder-chip-swap.md)); `NEW2` = eR PCB + second third-party chip (this session).
...
| 17-07 — left socket rebuilt: chip #2 (NEW2) onto eR PCB + spacer filed, gap 0.2 → 0.15 mm | legL = NEW2+mL @ 0.15 mm, legR = NEW1+mR (untreated) | SpotCheck run 4: hl.hx **-108.74\*, HEALTHY — no [E]** (first healthy hind hip-X since 01-07, first healthy mL reading ever); hr.hx **-8.15\* [E]**. Walking session: stair climbing up AND down + forward/backward walk on flat ground, all passed, NO recovery routine; green lights, misreads hr-only and immediately historic |

#### Interpretation

- **mL is not inherently dead.** It had flagged `[E]` under every board ever placed over it. it now reads healthy → the failure lived in the signal-path geometry (air gap vs disc strength), not in an unusable disc. The fix can be likely attributed to tigher gap, since eR previously did not flag any issues with mL before chip replacement, but the encoder replacement cannot be ruled out as a possible fix. untreated right socket is the discriminator. 
...
- **Working root-cause model — two-phase failure:**
  ...
  - **Falsifier to watch:** if the right socket's gap treatment does NOT clear `hr.hx`, NEW1's rework quality re-enters as a suspect.
...
>**Decision:** Filed down the spacer between the left socket's chip and disc to ~0.15 mm, and reworked a second third-party iC-MU chip onto the eR PCB. SpotCheck run 4 confirmed **hl.hx healthy (-108.74\*, no [E])**; the first healthy hind hip-X since 01-07 and the first healthy reading ever taken through mL. The right socket remains untreated as the confirmation experiment.

**Why:** To test the disc/air-gap hypothesis from the 15-07 board swap: if the right socket's NEW1 + mR at the old gap remains `[E]`, the left socket's success is attributable to the gap fix, not the chip replacement. If the right socket clears, then NEW1's rework quality is back in play as a suspect.

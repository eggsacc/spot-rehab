...
The assembled FX23 breakout turned out to probe the wrong interface — the encoder's RS-485 transceiver never reaches the FX23 connector (it routes through a separate connector into the driver board; only GND is common), so the bench-readout plan is retired. We instead repaired eL directly: a third-party iC-MU Y2HC chip (bought online) reworked onto the original eL PCB, everything reassembled to native config. The robot boots (after the usual jog-into-bounds trick), runs SpotCheck, walks, and — for the first time since the fault appeared — **survives a cold power cycle**. But SpotCheck now flags **BOTH hind hip-X encoders at <20% health**, the hind legs walk visibly bent inward, and both joints throw transient "Sensor misread" faults mid-walk. Neither flag is expected: the left has a brand-new chip, and the right (eR) had tested healthy on 01-07 — though only ever in the *left* socket.
...
- Consequence: the breakout can only observe the driver-board interface, not the raw encoder serial. Decided against designing a second breakout for the transceiver-side connector (lead time) — went for the direct repair instead.

#### eL repair — chip-level rework
- Bought a **third-party iC-Haus iC-MU chip, "Y2HC" marking** (trailing number differs from ours, but that number also differs between our own original left and right units → lot/date code, not a variant marker).
...
#### Interpretation — one fault, three symptoms
...
**Why:** The breakout physically cannot see the encoder's serial lines (wrong interface point — only GND is common with the transceiver), and a second breakout for the transceiver-side connector costs another design + fab cycle we didn't have time for.

**Alternatives considered:** Second breakout PCB for the transceiver-side connector — deferred indefinitely; probing the driver-board side of the FX23 — possible with the existing board but reads the processed bus, not the raw encoder output.
...
**Alternatives considered:** Whole replacement board from BD — no response path yet; swapping legs again — pointless (one bad part, two joints).
...
- [ ] Revisit the BD support-ticket scope once the above answers whether one or both sockets need parts.
